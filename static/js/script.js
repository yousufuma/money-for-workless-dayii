...
const HTTP = {
    address: "/",
    post: function (apiName = "", Body = {}, Print = false) {
        let _url = this.address + apiName + "?"
        if (apiName.substring(0, 5) == "[url]") _url = apiName
        let _body, _headers = {}, _sourceData;
        if (Body.FormData) {
            _body = Body.FormData
        } else {
        ...
        let print = { "URL": _url, "Body": Body };
        return new Promise(function (resolve, reject) {
            fetch(_url, { method: "POST", body: _body, headers: _headers })
                .then(response => {
                    _sourceData = response;
                    return response.text();
                })
                .then(res => {
                    if (HTTP.checkJson(res)) res = JSON.parse(res)
                    print.response = res
                    if (Print) console.log(print)
                    if (_sourceData.ok) resolve(res)
                    else { reject(res) }
                })
        });
    },
    generateURL: function (apiName, Options) {
        var url = this.address + apiName + "?";
        if (Options != null) { for (const key in Options) { url += "&" + key + "=" + Options[key] } }
        return url;
    },
